...
rvc_models_dir = "/content/drive/My Drive/COVER_AI"
...
if __name__ == '__main__':
    ...
    with gr.Blocks(title='AICoverGenWebUI') as app:
        # main tab
        with gr.Tab("Сгенерировать cover"):
            with gr.Row():
                with gr.Column():
                    with gr.Accordion(label='Загрузите ваше аудио'):
                        # YT
                        with gr.Column() as yt_link_col:
                            song_input = gr.Text(label='Ссылка на YouTube видеоролик с песней',
                                                 info='Вставьте ссылку на песню с YouTube или нажмите "Локальный файл" для загрузки аудио файла.')
                            with gr.Row():
                                show_file_upload_button = gr.Button('Локальный файл 🗃️')
                                show_file_audio_record_button = gr.Button('Запись аудио 🎙️')

                        # File
                        with gr.Column(visible=False) as file_upload_col:
                            local_file = gr.File(label='Аудио файл')
                            song_input_file = gr.UploadButton('Обзор 📂', file_types=['audio'], variant='primary')
                            song_input_file.upload(process_file_upload, inputs=[song_input_file],
                                                   outputs=[local_file, song_input])
                            with gr.Row():
                                show_yt_link_button = gr.Button('Ссылка на YouTube ▶')
                                show_file_audio_record_button = gr.Button('Запись аудио 🎙️')

                        # Audio Record
                        with gr.Column(visible=False) as audio_record_col:
                            local_file = gr.File(label='Аудио файл')
                            song_input_file = gr.UploadButton('Обзор 📂', file_types=['audio'], variant='primary')
                            song_input_file.upload(process_file_upload, inputs=[song_input_file],
                                                   outputs=[local_file, song_input])
                    ...
                    with gr.Accordion('Настройки'):
                        with gr.Row():
                            rvc_model = gr.Dropdown(voice_models, label='Голосовые модели', scale=5)
                            #ref_btn = gr.Button('🔁')
                        # Advance настройки
                        with gr.Accordion('Advance options', open=False):
                            with gr.Accordion('Тональность'):
                                pitch = gr.Slider(-20, 20, value=0, step=1, label='Высота тона',
                                                  info='Используйте 1 для женского голоса и -1 для мужского.')
                                pitch_all = gr.Slider(-12, 12, value=0, step=1, label='Общее изменение высоты тона',
                                                      info='Изменяет высоту звука / тональность вокала и инструментов одновременно. Изменение этого параметра немного снижает качество звука. (Полутона)')
                ...
                # Генерация
                with gr.Column():
                    ai_cover = gr.Audio(label='Результат', show_share_button=False)
                    with gr.Row():
                        clear_btn = gr.ClearButton(value='Очистить 🗑️',
